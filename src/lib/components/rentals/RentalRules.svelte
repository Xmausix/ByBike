<script>
	let { rules, lang } = $props();
	let title = 'WARUNKI WYPOŻYCZENIA ROWERÓW';
// 	let content = `Wypożyczający w momencie podpisania umowy, potwierdza przejęcie odpowiedzialności za wszystkie wypożyczone rowery, części i akcesoria oraz stwierdza, że rowery w momencie wypożyczenia były w nienagannym stanie.

// Przy okazji podpisywania umowy wypożyczenia roweru spisujemy Państwa dane oraz numer kontaktowy, więcej szczegółów w regulaminie najmu.

// W przypadku przebicia detki lub innych uszkodzeń powstałych z winy wypożyczającego, wypożyczający zobowiązany jest do samodzielnego dostarczenia uszkodzonego roweru do naszego biura przy ul. Chlebnickiej 19/20 w Gdańsku. W zależności od dostępności możemy zaoferować rower zastępczy tego samego typu, ponadto nie pokrywamy kosztów naprawy w innych warsztatach. Możesz zabezpieczyć się przed takimi niespodziankami, wypożyczając jeden z zestawów naprawczych, które umożliwią Ci samodzielną naprawę przebitej opony.`;

	function openTermsOfService() {
		alert('Tu możesz otworzyć regulamin lub przekierować użytkownika.');
	}
</script>

<div
	class="text-blue relative mx-auto flex max-w-7xl flex-col items-center px-6 py-16 font-sans lg:flex-row"
>
	<svg
		id="Layer_2"
		data-name="Layer 2"
	...
	<div class="flex min-h-[300px] flex-col justify-between space-y-8 pl-10 text-justify lg:flex-1">
		<h2 class="text-size-24 font-haboro-soft first-letter:text-size-30 font-bold">{title}</h2>
		<p class="font-open-sans font-light whitespace-pre-line">{rules}</p>
		<button
			class="bg-orange font-open-sans mt-auto self-end rounded-full px-8 py-1.5 font-semibold text-white transition hover:bg-orange-600"
